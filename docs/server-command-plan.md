...
输入 `/goto ` 后会补全当前在线玩家；坐标参数支持原版坐标形式，例如 `~ ~ ~`。它并不默认向所有普通玩家开放：仅配置文件 `gotoAllowedPlayers` 白名单中的名字可用，临时 OP 也可用。清空 OP 前必须先把可信玩家填入该数组。

`/goto` 只会移动执行者自身，不会赋予执行者 `/give`、`/gamemode`、`/kill` 或其他 OP 命令权限。默认配置中的白名单为空，表示停服部署后需要显式填写，不会意外开放传送。

::: warning 这是“自己去哪里”，不是管理员传送

`/goto` 不接受目标玩家参数，不能使用 `/goto @a ...` 把整服玩家传送，也不包含 `facing` 等管理员语法。需要与其他玩家协商传送时，使用 `/tpa`、`/tpahere`、`/tpaccept`。
